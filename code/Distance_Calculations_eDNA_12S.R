...
# I have to set the path  --- note you don't have to do this when using an R project 
  setwd("~/Documents/GitHub/biodiversity_edna")
...
#load geographic coordinates for sites and match up format between east and west coast ---------
  ...
  east <- read.csv("data/Site coordinates/eDNA_East_12S.csv")%>%
          mutate(Longitude=Longitude*-1)%>%
          st_as_sf(coords=c("Longitude","Latitude"),crs=latlong,remove=FALSE)%>%
          rename(lat=Latitude,
                 long=Longitude,
                 site_id=SampleName_12S)%>%
          mutate(coast="east")
    
  west <- read.csv("data/Site coordinates/eDNA_West_12S.csv")%>%
...
#Write the outputs ------------
    write.csv(east_lcp,"output/12S_east_lcp_distances.csv",row.names = TRUE)
    write.csv(east_gcd,"output/12S_east_gcd_distances.csv",row.names = TRUE)
    
    write.csv(west_lcp,"output/12S_west_lcp_distances.csv",row.names = TRUE)
    write.csv(west_gcd,"output/12S_west_gcd_distances.csv",row.names = TRUE)
